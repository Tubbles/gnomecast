    ...
    def __init__(self, fn, callback=None, _ffmpeg_output=None):
        ...
        def parse():
            ...
            self._ffmpeg_output = _ffmpeg_output if _ffmpeg_output else subprocess.check_output(
                ['ffmpeg', '-i', fn, '-f', 'ffmetadata', '-', '-f', 'mjpeg',
                    '-vframes', '1', '-ss', '27', '-vf', 'scale=600:-1', thumbnail_fn],
        ...
        if self.transcode:
            ...
            if self.audio_stream:
                self.transcode_cmd += ['-map', self.audio_stream.index, '-c:a',
                                       transcode_audio_to if self.transcode_audio else 'copy'] + (['-b:a', '256k'] if self.transcode_audio else [])
...
class Gnomecast(object):
    ...
    def __init__(self):
        self.ip = (([ip for ip in socket.gethostbyname_ex(socket.gethostname())[2] if not ip.startswith("127.")] or [[(s.connect(
            ("8.8.8.8", 53)), s.getsockname()[0], s.close()) for s in [socket.socket(socket.AF_INET, socket.SOCK_DGRAM)]][0][1]]) + [None])[0]
...
#      notes.append('Cast: %s' % self.last_known_player_state)
    ...
    def monitor_cast(self):
        while True:
            ...
            if not seeking and mc.status.player_state == 'PLAYING':
                GLib.idle_add(lambda: self.scrubber_adj.set_value(
    ...
    def update_media_button_states(self):
        ...
        self.play_button.set_image(Gtk.Image(stock=Gtk.STOCK_MEDIA_PAUSE)
                                   if self.cast and mc.status.player_state == 'PLAYING' else Gtk.Image(stock=Gtk.STOCK_MEDIA_PLAY))
            ...
            if self.cast and self.fn:
                ...
                for row in self.files_store:
                    ...
                    if not transcoder or self.cast != transcoder.cast or self.fn != transcoder.source_fn or self.audio_stream != transcoder.audio_stream:
                        self.transcoder = Transcoder(self.cast, fmd, self.video_stream, self.audio_stream, lambda did_transcode=None: GLib.idle_add(
                            self.update_status, did_transcode), self.error_callback, transcoder)
    ...
    def prep_next_transcode(self):
        ...
        for row in self.files_store:
            ...
            if transcode_next and not transcoder:
                ...
                transcoder = Transcoder(self.cast, fmd, fmd.video_streams[0] if fmd.video_streams else None, fmd.audio_streams[0] if fmd.audio_streams else None, lambda did_transcode=None: GLib.idle_add(
                    self.update_status, did_transcode), self.error_callback, transcoder)
                row[7] = transcoder
                transcode_next = False
